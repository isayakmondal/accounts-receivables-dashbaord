...
export default function EnhancedTable() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8080/invoice-management/getData")
      .then((resp) => {
  ...
  const handleClick = (event, row, doc_id) => {
                ...
                {stableSort(rows, getComparator(order, orderBy))
                  ...
                  .map((row, index) => {
                    ...
                    return (
                      <TableRow
                        hover
                        onClick={(event) => handleClick(event, row, row.doc_id)}
                        role="checkbox"
